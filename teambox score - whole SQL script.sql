...
-- team box score: Each game from 1996 to 2023, with team level data on those games (points scored etc.)
-- player box score: Each game from 1996 to 2023, with player level data on those games (points scored, assists etc.)
...
-- i then copied the output into notepad, and will refer to it when creating a table
...
INNER JOIN boxscore b ON b.gameid = tb_tp.gameid -- inner join only brings over games from boxscore that are within team_boxscore
...
select player_name, team, count(*) occurence_above_twenty_perc
